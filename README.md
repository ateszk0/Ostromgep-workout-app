...
- **Cloud Sync & Data Portability**: Back up and restore history, custom exercises, routines and body-weight data via Firebase — **merge** (union both sides, drop nothing) or overwrite, in either direction. Every destructive sync/import first writes a local auto-backup you can restore. Import/export full data as JSON, and **import a Hevy, Strong or FitNotes CSV export** to migrate your history in.
